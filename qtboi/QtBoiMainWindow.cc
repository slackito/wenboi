...
QtBoiMainWindow::QtBoiMainWindow(QWidget *parent)
  ...
  */
  ...
  connect(emuThread, &QtBoiEmuThread::emulationPaused, this,
          &QtBoiMainWindow::onPause);
  ...
  disassembly =
      new QtBoiDisassemblyWindow(centralWindow, &emuThread->gb, &tags);
  connect(disassembly, &QtBoiDisassemblyWindow::anchorClicked, this,
          &QtBoiMainWindow::onDisassemblyAnchorClicked);
...
void QtBoiMainWindow::createActions() {
  ...
  connect(emulatorPause, &QAction::triggered, emuThread,
          &QtBoiEmuThread::pause);
  connect(emulatorStep, &QAction::triggered, emuThread, &QtBoiEmuThread::step);
  connect(emulatorReset, &QAction::triggered, emuThread,
          &QtBoiEmuThread::reset);
  connect(limitFramerateAction, &QAction::triggered, this,
          &QtBoiMainWindow::onLimitFramerate);
  connect(viewDisassemblyWindow, &QAction::triggered, this,
          &QtBoiMainWindow::onViewDisassemblyWindow);
  connect(viewStatusWindow, &QAction::triggered, this,
          &QtBoiMainWindow::onViewStatusWindow);
  connect(scalingNone, &QAction::triggered, this,
          &QtBoiMainWindow::onScalingNone);
  connect(scalingQImage, &QAction::triggered, this,
          &QtBoiMainWindow::onScalingQImage);
  connect(scalingScale2X, &QAction::triggered, this,
          &QtBoiMainWindow::onScalingScale2X);
  connect(debugVideoDrawBackground, &QAction::triggered, this,
          &QtBoiMainWindow::onDebugVideoDrawBackground);
  connect(debugVideoDrawWindow, &QAction::triggered, this,
          &QtBoiMainWindow::onDebugVideoDrawWindow);
  connect(debugVideoDrawSprites, &QAction::triggered, this,
          &QtBoiMainWindow::onDebugVideoDrawSprites);
}
...
void QtBoiMainWindow::onLoadROM() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Load ROM"), "../roms", tr("GameBoy ROMs (*.gb *.gbc)"));
  if (filename == "")
    return;
...
void QtBoiMainWindow::onDisassemblyAnchorClicked(const QUrl &url) {
  ...
  if (url.scheme() == "gotoaddr") {
    ...
    tag = QInputDialog::getText(this, tr("Edit tag"),
                                tr("Enter the tag for the selected address"),
    ...
    QString comment = tag_comment.at(1);
    comment =
        QInputDialog::getText(this, tr("Edit comment"),
                              tr("Enter the comment for the selected address"),
...
void QtBoiMainWindow::onRedraw(const uchar *buffer) {
  ...
  if (dt > 1.0) {
    ...
    statusbar->showMessage(
        QString("%1 FPS, t=%2").arg(fps, 0, 'f', 2).arg(now()));
    last_FPS_update = now();
    frames_since_last_FPS_update = 0;
  }
  ...
  for (int y = 0; y < src_h; y++) {
    for (int x = 0; x < src_w; x++) {
      ...
      if (x < src_w - 1) {
        ...
        if (y > 0)
          C = src_pixels[src_w * (y - 1) + (x + 1)];
        else
          C = src_pixels[src_w * y + (x + 1)];
        if (y < src_h - 1)
          I = src_pixels[src_w * (y + 1) + (x + 1)];
        else
          I = src_pixels[src_w * y + (x + 1)];
      } else { // x==right border
        F = E;
        if (y > 0)
          C = src_pixels[src_w * (y - 1) + x];
        else
          C = F;
        if (y < src_h - 1)
          I = src_pixels[src_w * (y + 1) + x];
        else
          I = F;
      }
      if (B != H && D != F) {
...
void QtBoiMainWindow::onScalingNone() { scalingMethod = SCALING_NONE; }

void QtBoiMainWindow::onScalingQImage() { scalingMethod = SCALING_QIMAGE; }

void QtBoiMainWindow::onScalingScale2X() { scalingMethod = SCALING_SCALE2X; }
...
void QtBoiMainWindow::onPause() {
  ...
  status->update();
  statusbar->showMessage(
      tr("Emulation paused", "Status bar emulation paused msg"));
}

void QtBoiMainWindow::onResume() {
  statusbar->showMessage(
      tr("Emulation running", "Status bar emulation running msg"));
  last_FPS_update = 0;
  frames_since_last_FPS_update = 0;
}
...
double QtBoiMainWindow::now() { return time.elapsed() / 1000.0; }
