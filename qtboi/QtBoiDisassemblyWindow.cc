...
std::string
QtBoiDisassemblyWindow::operandToHtml(const Instruction::Operand &op) {
  std::string result;
  switch (op.type) {
...
std::string QtBoiDisassemblyWindow::insToHtml(const Instruction &ins) {
  ...
  if (ins.type == Instruction::RESET) {
    ...
    if (ins.str.substr(0, 2) == "JR") {
      ...
      result += "]";
    } else if ((ins.str.substr(0, 2) == "JP" &&
                ins.op1.type == Instruction::INM16) ||
...
void QtBoiDisassemblyWindow::gotoAddress(u16 addr) {
  ...
  str << "<tr bgcolor=#c0ffc0><td "
         "width=25%>Labels</td><td>&nbsp;BP&nbsp;</td><td>&nbsp;Addr&nbsp;</td>"
         "<td>&nbsp;Opcodes&nbsp;</td><td>Instruction</td><td>Comments</td></"
         "tr>";
  ...
  while (pos < end) {
    ...
    str << "<tr bgcolor="
        << (isBP ? "#ff0000"
                 : (pos == gb->regs.PC ? "#ffc0c0"
                                       : (hilightBG ? "#d0d0d0" : "#ffffff")))
        << ">" << "<td>" << tag.toStdString() << (tag == "" ? "" : ":")
        << "&nbsp;</td>" << "<td>&nbsp;<a href=\"togglebp:" << std::dec << pos
        << "\">" << (isBP ? "#" : "&nbsp;") << "</a>&nbsp;</font></td>"
        << "<td><a href=\"tag:" << std::dec << pos << "\">0x" << std::hex
        << std::setw(4) << std::setfill('0') << pos
        << "</a>&nbsp;&nbsp;</td><td>";
    ...
    str << "<td>" << "<a href=\"comment:" << std::dec << pos
        << "\">#</a>&nbsp;&nbsp;" << comment.toStdString()
        << "&nbsp;&nbsp;</td></tr>";
    pos += ins.length;
  ...
  if (historyPosition < history.size() - 1 &&
      history[historyPosition + 1] == addr)
    historyPosition++;
  ...
  if (historyPosition >= 0 && historyPosition < history.size() - 1)
    forwardButton->setEnabled(true);
  else
    forwardButton->setEnabled(false);
}

void QtBoiDisassemblyWindow::gotoPC() { gotoAddress(gb->regs.PC); }

void QtBoiDisassemblyWindow::refresh() { gotoAddress(currentAddress); }
...
void QtBoiDisassemblyWindow::onGotoButton() {
  QString s = QInputDialog::getText(this, tr("Go to address"),
                                    tr("Enter the address to disassemble"));
