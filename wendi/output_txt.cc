...
void TextDisassemblyOutput::show_disassembly_block(const CodeBlock &b) {
  ...
  for (CodeBlock::XrefsConstIterator i = b.xrefs.begin(); i != b.xrefs.end();
       i++) {
    xrefsstr << std::hex << "0x" << std::setw(4) << std::setfill('0')
             << i->first << ",";
  }
  ...
  for (CodeBlock::DisassemblyConstIterator i = b.disassembly.begin();
    ...
    out << std::hex << "\t0x" << std::setw(4) << std::setfill('0') << i->first
        << "\t" << i->second << std::endl;
  }
}

void TextDisassemblyOutput::show_jump_table_block(GameBoy &gb,
                                                  const CodeBlock &b) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    out << "\t0x" << std::hex << std::setw(4) << std::setfill('0') << addr
        << "\t" << i << "\t0x" << std::setw(2) << gb.memory.read16(addr)
        << std::endl;
    ;
  }
}
...
void TextDisassemblyOutput::hexdump(GameBoy &gb, address start, address end) {
  if (end > start) {
    ...
    if (i < start) {
      out << std::endl
          << ";" << std::hex << "0x" << std::setw(4) << std::setfill('0')
          << start << " ";
      while (i < start) {
        if (i % 0x10 == 8)
          out << "- ";
