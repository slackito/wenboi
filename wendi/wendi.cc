...
void classify_block(CodeBlock &b) {
  ...
  for (CodeBlock::XrefsConstIterator i = b.xrefs.begin(); i != b.xrefs.end();
       i++) {
  ...
  if (b.type & CodeBlock::ENTRYPOINT)
    ...
    if (b.type & (CodeBlock::VBLANK_HANDLER | CodeBlock::LCD_STAT_HANDLER |
                  CodeBlock::TIMER_HANDLER | CodeBlock::SERIAL_HANDLER |
    ...
    block_name << "_" << std::hex << std::setw(4) << std::setfill('0')
               << b.start;
  }
...
bool is_jump(const Instruction &ins) {
  if (ins.type == Instruction::UNCONDITIONAL_JUMP ||
      ...
      ins.type == Instruction::CALL || ins.type == Instruction::RESET)
    return true;
  return false;
}

bool is_block_end(const Instruction &ins) {
  if (is_jump(ins) || ins.type == Instruction::CONDITIONAL_RET ||
...
list<CodeBlock>::iterator find_block(list<CodeBlock> &l, address addr) {
  ...
  while (i != l.end()) {
    // logger.trace("finding block. addr=0x", std::hex, addr, " start=0x",
    // i->start, " end=0x", i->end);
...
void new_block_start(address dst, address src,
                     Instruction::InstructionType type, CodeBlock &current,
  ...
  } else if (dst > current.start &&
             dst < current.end) // Check if dst is inside this block
  {
    logger.info("Splitting current block 0x", std::hex, current.start, " at 0x",
                dst);
  ...
  while (!pending.empty()) {
    ...
    while (!block_end) {
      ...
      if (find_block(blocks, new_addr) != blocks.end() ||
          find_block(pending, new_addr) != pending.end() || is_block_end(ins)) {
        block_end = true;
        if (does_fall_through(ins)) {
          new_block_start(new_addr, addr, Instruction::OTHER, block, blocks,
                          pending);
        }
      }
