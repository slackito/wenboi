...
GBVideo::GBVideo(GameBoy *core)
    : OAM(), core(core), cur_window_line(0), mode(2), OAM_BUSY(false),
      VRAM_BUSY(false), frames_rendered(0), screen(0), oldscreen(0),
      newscreen(0), display_mode(NORMAL), background_enabled(true),
      window_enabled(true), sprites_enabled(true), cycles_until_next_update(0) {
  oldscreen = new u8[160 * 144];
  newscreen = new u8[160 * 144];
  screen =
      new u8[160 * 144]; // sum of old and new, 8 possible "colors" per pixel
}
...
void GBVideo::reset() { cycles_until_next_update = 0; }
...
u8 GBVideo::read_VRAM(int addr) const {
  if (!VRAM_BUSY || !check_bit(core->memory.high[GBMemory::I_LCDC], 7))
...
u8 GBVideo::read_OAM(int addr) const {
  if (!OAM_BUSY || !check_bit(core->memory.high[GBMemory::I_LCDC], 7))
...
u32 GBVideo::update() {
  ...
  case 2: {
    ...
    if (LY == LYC) {
      ...
      if (check_bit(STAT, 6)) {
        logger.trace("Requesting IRQ_LCD_STAT -- LY = LYC = ", LY,
                     " EI=", int(core->memory.IE), " IME =", int(core->IME));
        core->irq(GameBoy::IRQ_LCD_STAT);
      }
    }
...
void GBVideo::draw() {
  ...
  if (LY < 144 && display_mode == NORMAL) {
    ...
    if (background_enabled && check_bit(LCDC, 0)) // is BG display active?
    {
      ...
      for (int x = 0; x < 160; x++) {
        ...
        u8 current_tile_index =
            VRAM[tile_map_addr + 32 * map_y + map_x] + tile_data_base;
    ...
    //  Draw the window
    if (window_enabled && check_bit(LCDC, 5) && LY > WY &&
        LY < 144) // is BG display active?
    {
      ...
      logger.trace("Drawing window at (", WX, ",", WY, ")\tLY=", LY,
                   " cur_window_line=", cur_window_line);
    ...
    if (sprites_enabled && check_bit(LCDC, 1)) {
      ...
      for (u32 i = 0; i < v.size() && i < 10; i++) {
        ...
        logger.trace("sprite #", i, "; screen_x = ", int(screen_x),
        ...
        for (int x = sprite_x; x < max_sprite_x; x++) {
          ...
          if (color != 0) {
            if (newscreen[line_base + screen_x + x] >> 4 == 0 &&
              ...
              newscreen[line_base + screen_x + x] =
                  color | ((pal_num + 1) << 4);
            }
          }
