...
void log_rom_header(GBRom *rom, Logger::log_level level) {
  ...
  out << "Entrypoint: " << std::setfill('0') << std::setw(2)
      << int(rom->header.entry_point[0]) << " " << std::setw(2)
      << int(rom->header.entry_point[1]) << " " << std::setw(2)
      << int(rom->header.entry_point[2]) << " " << std::setw(2)
      << int(rom->header.entry_point[3]) << " " << endl;
  ...
  out << std::hex;
  out << "Destination code: 0x" << std::setw(2)
      << int(rom->header.destination_code) << endl;
  if (rom->header.old_licensee_code == 0x33) {
    ...
    out << "Old licensee code: 0x" << std::setw(2)
        << int(rom->header.old_licensee_code) << endl;
  }
  ...
  out << "Mask ROM version number: 0x" << std::setw(2)
      << int(rom->header.mask_rom_version_number) << endl;
  out << "Header checksum: 0x" << std::setw(2)
      << int(rom->header.header_checksum) << endl;
