...
struct Instruction {
  ...
  enum OperandType { NONE = 0, REG, MEM_DIRECT, MEM_INDIRECT, INM8, INM16 };
  ...
  Instruction(int length, InstructionType type, u8 opcode, u8 sub_opcode,
      ...
      : length(length), type(type), all(all), opcode(opcode),
        sub_opcode(sub_opcode), str(opcode_str), op1(op1), op2(op2) {}
};
