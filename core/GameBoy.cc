...
bool GameBoy::is_pad[NUM_CONTROLS] = {false, false, false, false,
                                      true,  true,  true,  true};

GameBoy::GameBoy(GameBoyType type)
    : gameboy_type(type), memory(this), video(this), rom(0), regs(), IME(1),
      HALT(false), STOP(false), cycle_count(0),
      cycles_until_next_instruction(0), divider_count(0), timer_count(0),
      breakpoints(), last_breakpoint_id(0) {}

GameBoy::~GameBoy() { free_ROM(); }
...
void GameBoy::delete_breakpoint(int id) { breakpoints.erase(id); }

void GameBoy::enable_breakpoint(int id) { breakpoints[id].enabled = true; }

void GameBoy::disable_breakpoint(int id) { breakpoints[id].enabled = false; }

#include "opcodes.h"

GameBoy::run_status GameBoy::run_cycle() {

  // if (HALT) logger.critical("(HALT) cycles_until_next_instruction = ",
  ...
  if (cycles_until_next_instruction <= 0) {
    ...
    // bugfix (blargg test 02-halt): cpu exits from halt state if there is an
    // interrupt flag raised, even if interrupts are globally disabled
    ...
    if (!(HALT || STOP)) {
      ...
      case 0x00:
        ...
        for_each_register(0x3E, 0x06, 0x0E, 0x16, 0x1E, 0x26, 0x2E, LD_reg_nn)
            ...
            // LD r1,r2
            for_each_register(0x7F, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D,
                              LD_A_reg) for_each_register(0x47, 0x40, 0x41,
                                                          0x42, 0x43, 0x44,
                                                          0x45, LD_B_reg)
                for_each_register(0x4F, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D,
                                  LD_C_reg)
                    for_each_register(0x57, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55,
                                      LD_D_reg)
                        for_each_register(0x5F, 0x58, 0x59, 0x5A, 0x5B, 0x5C,
                                          0x5D, LD_E_reg)
                            for_each_register(0x67, 0x60, 0x61, 0x62, 0x63,
                                              0x64, 0x65, LD_H_reg)
                                for_each_register(0x6F, 0x68, 0x69, 0x6A, 0x6B,
                                                  0x6C, 0x6D, LD_L_reg)

            // LD reg, (HL)
            for_each_register(0x7E, 0x46, 0x4E, 0x56, 0x5E, 0x66, 0x6E,
                              LD_reg__HL_)

            // LD (HL), reg
            for_each_register(0x77, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75,
                              LD__HL__reg)
        ...
        for_each_register16(0x09, 0x19, 0x29, 0x39, ADD_HL_reg16)
          ...
          reset_flag(HALF_CARRY_FLAG); // GBCPUman.pdf contradicts previous
                                       // reference :P
        ...
        case 0x10: {
          ...
          if (sub_opcode == 0x00) {
            ...
            logger.critical("Unknown sub-opcode ", std::hex, std::setw(2),
                            std::setfill('0'), sub_opcode, " after 0x10. PC=0x",
                            regs.PC - 2);
          }
          cycles_until_next_instruction = 4;
          break;
      ...
      case 0x80:
        ...
        for_each_register(0x8F, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, ADC_A_reg)
          ...
          int half_res =
              (regs.A & 0x0F) + (memory.read(regs.HL) & 0x0F) + carry;
          regs.A = static_cast<u8>(res);
        ...
        case 0xCB: {
          ...
          default: {
            ...
            default:
              logger.critical("Unknown sub-opcode after 0xCB. PC=0x", std::hex,
                              regs.PC - 2);
              break;
            }
          }
...
void GameBoy::release_control(Control b) { controls[b] = false; }
...
std::string GameBoy::status_string() {
  ...
  result << "t = " << std::dec << cycle_count << "\tPC = " << std::hex
         << std::setw(4) << std::setfill('0') << regs.PC << "\t" << ins.all
         << std::endl
         << "A = " << std::hex << std::setw(2) << std::setfill('0')
         << int(regs.A) << " B = " << std::hex << std::setw(2)
         << std::setfill('0') << int(regs.B) << " C = " << std::hex
         << std::setw(2) << std::setfill('0') << int(regs.C)
         << " D = " << std::hex << std::setw(2) << std::setfill('0')
         << int(regs.D) << " E = " << std::hex << std::setw(2)
         << std::setfill('0') << int(regs.E) << " H = " << std::hex
         << std::setw(2) << std::setfill('0') << int(regs.H)
         << " L = " << std::hex << std::setw(2) << std::setfill('0')
         << int(regs.L) << "\tflags = " << int(regs.flags)
         << "\tZF = " << check_flag(ZERO_FLAG) << std::endl
         << "AF = " << std::hex << std::setw(4) << std::setfill('0')
         << int(regs.AF) << " BC = " << std::hex << std::setw(4)
         << std::setfill('0') << int(regs.BC) << " DE = " << std::hex
         << std::setw(4) << std::setfill('0') << int(regs.DE)
         << " HL = " << std::hex << std::setw(4) << std::setfill('0')
         << int(regs.HL) << "\tSP = " << std::hex << std::setw(4)
         << std::setfill('0') << int(regs.SP) << std::endl
         << "IME = " << int(IME)
         << " IE = " << int(memory.read(0xFFFF, GBMemory::DONT_WATCH))
...
void GameBoy::update_JOYP() {
  ...
  if (check_bit(JOYP, 4) == false) {
    if (controls[PAD_DOWN])
      JOYP = reset_bit(JOYP, 3);
    else
      JOYP = set_bit(JOYP, 3);
    if (controls[PAD_UP])
      JOYP = reset_bit(JOYP, 2);
    else
      JOYP = set_bit(JOYP, 2);
    if (controls[PAD_LEFT])
      JOYP = reset_bit(JOYP, 1);
    else
      JOYP = set_bit(JOYP, 1);
    if (controls[PAD_RIGHT])
      JOYP = reset_bit(JOYP, 0);
    else
      JOYP = set_bit(JOYP, 0);
  } else // Write button status
  {
    if (controls[BUTTON_START])
      JOYP = reset_bit(JOYP, 3);
    else
      JOYP = set_bit(JOYP, 3);
    if (controls[BUTTON_SELECT])
      JOYP = reset_bit(JOYP, 2);
    else
      JOYP = set_bit(JOYP, 2);
...
std::string GameBoy::get_port_name(int port) {
  ...
  case 0x12:
    ...
    break;
  case 0x13:
    port_name = "CH1_FREQ_LO";
    break;
  case 0x14:
    port_name = "CH1_FREQ_HI_KICK";
    break;
  case 0x16:
    port_name = "CH2_WAVE";
    break;
  case 0x17:
    ...
    break;
  case 0x18:
    port_name = "CH2_FREQ_LO";
    break;
  case 0x19:
    port_name = "CH2_FREQ_HI_KICK";
    break;
  case 0x1A:
    port_name = "CH3_ONOFF";
    break;
  case 0x1C:
    port_name = "CH3_VOLUME";
    break;
  case 0x1D:
    port_name = "CH3_FREQ_LO";
    break;
  case 0x1E:
    port_name = "CH3_FREQ_HI_KICK";
    break;
  case 0x21:
    port_name = "CH4_ENV";
    break;
...
Instruction GameBoy::disassemble_opcode(u16 addr) {
  ...
  switch (opcode) {
  // LD n, nn
  dis_for_each_register(0x3E, 0x06, 0x0E, 0x16, 0x1E, 0x26, 0x2E, "LD",
                        Instruction::LOAD, dis_reg_inm)

      // LD r1,r2
      dis_for_each_register(0x7F, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, "LD",
                            Instruction::LOAD, dis_A_reg)
          dis_for_each_register(0x47, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, "LD",
                                Instruction::LOAD, dis_B_reg)
              dis_for_each_register(0x4F, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D,
                                    "LD", Instruction::LOAD, dis_C_reg)
                  dis_for_each_register(0x57, 0x50, 0x51, 0x52, 0x53, 0x54,
                                        0x55, "LD", Instruction::LOAD,
                                        dis_D_reg)
                      dis_for_each_register(0x5F, 0x58, 0x59, 0x5A, 0x5B, 0x5C,
                                            0x5D, "LD", Instruction::LOAD,
                                            dis_E_reg)
                          dis_for_each_register(0x67, 0x60, 0x61, 0x62, 0x63,
                                                0x64, 0x65, "LD",
                                                Instruction::LOAD, dis_H_reg)
                              dis_for_each_register(
                                  0x6F, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D,
                                  "LD", Instruction::LOAD, dis_L_reg)

      // LD reg, (HL)
      dis_for_each_register(0x7E, 0x46, 0x4E, 0x56, 0x5E, 0x66, 0x6E, "LD",
                            Instruction::LOAD, dis_reg__HL_)

      // LD (HL), reg
      dis_for_each_register(0x77, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, "LD",
                            Instruction::LOAD, dis__HL__reg)

          dis__reg16__inm(0x36, "LD", Instruction::LOAD, HL)

              dis_reg__reg16_(0x0A, "LD", Instruction::LOAD, A, BC)
                  dis_reg__reg16_(0x1A, "LD", Instruction::LOAD, A,
                                  DE) dis_reg__inm_(0xFA, "LD",
                                                    Instruction::LOAD, A)
  ...
  case 0xE0: {
    ...
    result << "LD (0xFF" << std::setw(2) << port << "), A" << "\t["
           << get_port_name(port) << "]";
    opcode_str = "LDH";
    ins_type = Instruction::LOAD;
  ...
  case 0xF0: {
    int port = int(this->memory.read(PC++, GBMemory::DONT_WATCH));
    result << "LD A, (0xFF" << std::setw(2) << port << ")" << "\t["
           << get_port_name(port) << "]";
    opcode_str = "LDH";
    ins_type = Instruction::LOAD;
    op1.str = "A";
  ...
  dis__inm__reg16(0x08, "LD", Instruction::LOAD, SP)
      ...
      // 8-bit ALU
      // ADD A,reg
      dis_for_each_register(0x87, 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, "ADD",
                            Instruction::ALU, dis_A_reg)

          dis_reg__reg16_(0x86, "ADD", Instruction::ALU, A, HL)
              dis_reg_inm(0xC6, "ADD", Instruction::ALU, A)

      // ADC A, n
      dis_for_each_register(0x8F, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, "ADC",
                            Instruction::ALU, dis_A_reg)
      ...
      // SUB n
      dis_for_each_register(0x97, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, "SUB",
                            Instruction::ALU, dis_reg)

          dis__reg16_(0x96, "SUB", Instruction::ALU, HL)
              dis_inm8(0xD6, "SUB", Instruction::ALU)

      // SBC n
      dis_for_each_register(0x9F, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, "SBC",
                            Instruction::ALU, dis_reg)
      ...
      // AND n
      dis_for_each_register(0xA7, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, "AND",
                            Instruction::ALU, dis_reg)
      ...
      // OR n
      dis_for_each_register(0xB7, 0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5, "OR",
                            Instruction::ALU, dis_reg)
      ...
      // XOR n
      dis_for_each_register(0xAF, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC, 0xAD, "XOR",
                            Instruction::ALU, dis_reg)
      ...
      // CP n
      dis_for_each_register(0xBF, 0xB8, 0xB9, 0xBA, 0xBB, 0xBC, 0xBD, "CP",
                            Instruction::ALU, dis_reg)
      ...
      // INC n
      dis_for_each_register(0x3C, 0x04, 0x0C, 0x14, 0x1C, 0x24, 0x2C, "INC",
                            Instruction::ALU, dis_reg)

          dis__reg16_(0x34, "INC", Instruction::ALU, HL)

      // DEC n
      dis_for_each_register(0x3D, 0x05, 0x0D, 0x15, 0x1D, 0x25, 0x2D, "DEC",
                            Instruction::ALU, dis_reg)
      ...
      // 16-bit ALU
      // ADD HL, n
      dis_for_each_register16(0x09, 0x19, 0x29, 0x39, "ADD", Instruction::ALU,
                              dis_HL_reg16)
      ...
      // INC nn
      dis_for_each_register16(0x03, 0x13, 0x23, 0x33, "INC", Instruction::ALU,
                              dis_reg16)

      // DEC nn
      dis_for_each_register16(0x0B, 0x1B, 0x2B, 0x3B, "DEC", Instruction::ALU,
                              dis_reg16)
    ...
    switch (sub_opcode) {
      ...
      dis_for_each_register(0x37, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, "SWAP",
          ...
          // RLC n
          dis_for_each_register(0x07, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, "RLC",
                                Instruction::ALU, dis_reg)
          ...
          // RL n (through carry)
          dis_for_each_register(0x17, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, "RL",
                                Instruction::ALU, dis_reg)
          ...
          // RRC n
          dis_for_each_register(0x0F, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, "RRC",
                                Instruction::ALU, dis_reg)
          ...
          // RR n (through carry)
          dis_for_each_register(0x1F, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, "RR",
                                Instruction::ALU, dis_reg)
          ...
          // SLA n
          dis_for_each_register(0x27, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, "SLA",
                                Instruction::ALU, dis_reg)
          ...
          // SRA n
          dis_for_each_register(0x2F, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, "SRA",
                                Instruction::ALU, dis_reg)
          ...
          // SRL n
          dis_for_each_register(0x3F, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, "SRA",
                                Instruction::ALU, dis_reg)
  ...
  dis(0x27, "DAA", Instruction::ALU)
      ...
      dis_inm16(0xC2, "JP NZ", Instruction::CONDITIONAL_JUMP)
          dis_inm16(0xCA, "JP Z", Instruction::CONDITIONAL_JUMP)
              dis_inm16(0xD2, "JP NC", Instruction::CONDITIONAL_JUMP) dis_inm16(
                  0xDA, "JP C", Instruction::CONDITIONAL_JUMP) case 0xE9:
    result << "JP (HL)";
    opcode_str = "JP";
  ...
  return Instruction(PC - addr, ins_type, opcode, sub_opcode, result.str(),
                     opcode_str, op1, op2);
}
