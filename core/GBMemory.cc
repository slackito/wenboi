...
void GBMemory::delete_watchpoint(int id) { watchpoints.erase(id); }

void GBMemory::enable_watchpoint(int id) { watchpoints[id].enabled = true; }

void GBMemory::disable_watchpoint(int id) { watchpoints[id].enabled = false; }

void GBMemory::check_watchpoints(u16 addr, u16 value) {
  for (WatchpointMap::iterator i = watchpoints.begin(); i != watchpoints.end();
       i++) {
...
void GBMemory::write(u16 addr, u8 value, WatchpointControl watch) {
  ...
  if (addr < 0x8000)
    ...
    core->video.write_VRAM(addr, value);
  else if (addr < 0xC000)
    mbc->write(addr, value);
  else if (addr < 0xE000)
    ...
    core->video.write_OAM(addr, value);
  else if (addr == 0xFF00) {
    ...
    if (port_access[port_number] != READ_ONLY) {
      ...
      if (addr == DIV) {
        ...
        u16 dma_src = value << 8;
        // logger.warning("OAM DMA transfer from 0x", std::hex,
        // std::setfill('0'), dma_src, " requested");
        core->video.DMA_OAM(dma_src);
      }
    } else {
      logger.warning("WTF? Trying to write to addr=0x", std::hex, addr,
                     " (read-only port). PC=0x", std::hex, core->regs.PC);
    }

  } else {
    std::ostringstream errmsg;
    errmsg << "Invalid write address 0x" << std::hex << std::setw(4)
           << std::setfill('0') << addr;
...
u8 GBMemory::read(u16 addr, WatchpointControl watch) {
  ...
  if (addr < 0x8000)
    ...
    return core->video.read_VRAM(addr);
  else if (addr < 0xC000)
    return mbc->read(addr);
  else if (addr < 0xE000)
    ...
    return core->video.read_OAM(addr);
  else if (addr >= 0xFF00) {
    int port_number = addr - 0xFF00;
    // Check access
    if (port_access[port_number] == WRITE_ONLY) {
      logger.warning("WTF? Trying to read from addr=0x", std::hex, addr,
                     " (write-only port). PC=0x", std::hex, core->regs.PC);
    }
    return high[port_number];
  } else {
    std::ostringstream errmsg;
    errmsg << "Invalid read address 0x" << std::hex << std::setw(4)
           << std::setfill('0') << addr;
...
u16 GBMemory::read16(u16 addr, WatchpointControl watch) {
  ...
  if (addr < 0x8000)
    return mbc->read16(addr);
  else if (addr < 0xA000)
    return core->video.read16_VRAM(addr);
  else if (addr < 0xC000)
    return mbc->read16(addr);
  else if (addr < 0xE000)
    return WRAM[addr - WRAM_BASE] + (WRAM[addr - WRAM_BASE + 1] << 8);
  else if (addr < 0xFDFF)
    return read16(addr - 0x2000);
  else if (addr < 0xFEA0)
    return core->video.read16_OAM(addr);
  else if (addr >= 0xFF00)
    return high[addr - 0xFF00] + (high[addr - 0xFF00 + 1] << 8);
  else {
    std::ostringstream errmsg;
    errmsg << "Invalid read address 0x" << std::hex << std::setw(4)
           << std::setfill('0') << addr;
