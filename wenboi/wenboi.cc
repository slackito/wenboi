...
int main(int argc, char **argv) {
  ...
  SDL_Surface *display =
      SDL_SetVideoMode(320, 288, 32, SDL_SWSURFACE | SDL_DOUBLEBUF);
  ...
  while (true) {
    if (first_cmd) {
      ...
      if (!cin.good())
        break; // if stdin is closed, exit main loop
      if (line == "")
    ...
    if (command == "step" || command == "s") {
      int status;
      while ((status = gb.run_cycle()) == GameBoy::WAIT) {
      } // do nothing
      ...
      while (pos < end) {
        ...
        cout << "0x" << std::hex << std::setw(4) << std::setfill('0') << pos
             << "\t";
      ...
      cout << "[" << std::hex << std::setw(4) << std::setfill('0') << addr
           << "]\t";
      if (arguments.size() > 1) {
        if (arguments[1] == "d")
      ...
      int addr = str2int(arguments[0]);
      cout << "breakpoint #" << gb.set_breakpoint(addr) << " set at 0x"
           << std::hex << std::setw(4) << std::setfill('0') << addr << endl;
      ...
      int addr = str2int(arguments[0]);
      cout << "watchpoint #" << gb.memory.set_watchpoint(addr) << " set at 0x"
           << std::hex << std::setw(4) << std::setfill('0') << addr << endl;
